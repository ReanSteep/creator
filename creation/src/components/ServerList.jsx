...
const initialServers = [
  { id: 1, name: "AI Club", ownerId: 1 },
  { id: 2, name: "Gaming", ownerId: 2 },
  { id: 3, name: "Music", ownerId: 3 },
];
...
export default function ServerList() {
  ...
  const handleAddServer = () => {
    if (newServerName.trim()) {
      setServers([
        ...servers,
        { id: Date.now(), name: newServerName.trim(), ownerId: myUserId }
      ]);
      setNewServerName("");
      setShowModal(false);
    }
    ...
    <>
      <div style={{ background: "#23272b", minHeight: "100vh", width: 80, display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 20, position: "relative" }}>
        ...
        {servers.map(server => {
          ...
          const isMine = server.ownerId === myUserId;
          return (
            <div
              key={server.id}
              draggable
            ...
            >
              <Link
                ...
                title={isMine ? "Your server" : undefined}
              >
                {server.name[0]}
                {isMine && (
